...
const TimelineCalendarContext = /*#__PURE__*/React.createContext(undefined);
const TimelineProvider = props => {
  ...
  /** Prepare data*/
  const pages = useMemo(() => calculateDates(firstDay, minDate, maxDate, initialDate.current, timeZone), [firstDay, minDate, maxDate, timeZone]);
  ...
  const hours = useMemo(() => calculateHours(start, end, timeInterval, hourFormat), [end, start, timeInterval, hourFormat]);

  /** Animated value */
  const currentIndex = useSharedValue(pages[viewMode].index);
  const startDate = useDerivedValue(() => pages[viewMode].data[currentIndex.value]);
  const timeIntervalHeight = useSharedValue(initialTimeIntervalHeight);
  const minTimeIntervalHeight = useSharedValue(initialMinTimeIntervalHeight || 0);
  const isDragCreateActive = useSharedValue(false);
  const offsetY = useSharedValue(0);
  const rHourWidth = useMemo(() => PixelRatio.roundToNearestPixel(hourWidth), [hourWidth]);
  ...
  const [currentDate, setCurrentDate] = useState(() => getCurrentDate(timeZone));
  ...
  return /*#__PURE__*/React.createElement(TimelineCalendarContext.Provider, {
    value: value
  }, children);
};
