...
const useDragCreateGesture = _ref => {
  ...
  const _onEnd = event => {
    ...
    const eventStart = moment.tz(startDate, tzOffset).add(positionIndex, 'd').add(time, 'h').add(start, 'h');
  ...
  useAnimatedReaction(() => gestureEvent.value, event => {
    if (!event) {
      return;
    }
    const {
  ...
  const dragCreateGesture = Gesture.Pan().minPointers(1).manualActivation(true).onTouchesDown((event, stateManager) => {
  ...
  useAnimatedReaction(() => isTouchesUp.value, touchesUp => {
    if (touchesUp) {
      runOnJS(_onEnd)({
  ...
  useAnimatedReaction(() => isDragCreateActive.value, active => {
