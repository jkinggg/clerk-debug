...
const DragEditItem = _ref => {
  ...
  const currentHour = (0, _reactNativeReanimated.useSharedValue)(event.top / timeIntervalHeight.value + start);
  ...
  const recalculateEvent = () => {
    ...
    const newEvent = {
      ...
      end: currentDateMoment.clone().add(eventHeight.value / timeIntervalHeight.value, 'h').toISOString()
    };
  ...
  const dragPositionGesture = _reactNativeGestureHandler.Gesture.Pan().enabled(isEnabled).runOnJS(true).maxPointers(1).onStart(() => {
    ...
    const newTopPosition = roundedHour * timeIntervalHeight.value + spaceFromTop;
  ...
  }, renderEventContent ? renderEventContent(event, timeIntervalHeight) : _renderEventContent(), /*#__PURE__*/_react.default.createElement(_reactNativeGestureHandler.GestureDetector, {
